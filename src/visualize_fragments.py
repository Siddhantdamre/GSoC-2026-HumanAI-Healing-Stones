...
def process_and_view(file_path):
    try:
        ...
        print(f"  - Downsampling (3cm resolution)...")
        down_pcd = pcd.uniform_down_sample(10)
        down_pcd = down_pcd.voxel_down_sample(voxel_size=0.03)
...
if __name__ == "__main__":
    try:
        ...
        all_ply_files = glob.glob(os.path.join(base_dir, "*.PLY")) + glob.glob(os.path.join(base_dir, "*.ply"))
        
        ply_files = []
        for f in all_ply_files:
            bname = os.path.basename(f).lower()
            if "combined" not in bname and "reconstructed" not in bname:
                ply_files.append(f)
        
        if not ply_files:
            all_ply_files = glob.glob("*.PLY") + glob.glob("*.ply")
            for f in all_ply_files:
                bname = os.path.basename(f).lower()
                if "combined" not in bname and "reconstructed" not in bname:
                    ply_files.append(f)
