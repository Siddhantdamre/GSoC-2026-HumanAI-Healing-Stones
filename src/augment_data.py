...
if __name__ == "__main__":
    ...
    print("Downsampling huge 500MB raw scan to 5cm resolution for efficient augmentation...")
    base_pcd = base_pcd.uniform_down_sample(every_k_points=10).voxel_down_sample(voxel_size=0.05)
    ...
    for i, frag in enumerate(fragments):
        # --- Estimate Curvature ---
        frag.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.05, max_nn=30))
        frag.estimate_covariances(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.05, max_nn=30))
        covs = np.nan_to_num(np.asarray(frag.covariances))
        if len(covs) > 0:
            evals = np.linalg.eigvalsh(covs) # Ascending order, evals[:, 0] is smallest
            curvatures = evals[:, 0] / (np.sum(evals, axis=1) + 1e-6)
        else:
            curvatures = np.zeros(len(frag.points))
        
        frag_name = f"fragment_{i:03d}.ply"
        curv_name = f"fragment_{i:03d}_curvature.npy"
        np.save(os.path.join(output_dir, curv_name), curvatures)

        # 1. Generate random transform OR Targeted Planar Slide
        if np.random.rand() < 0.3:
            # Planar slide (False Positive Generation)
            normals = np.asarray(frag.normals)
            # Find the most common normal direction (the dominant flat plane)
            # Since normals can be flipped, use covariance of normals
            cov_n = np.cov(normals.T)
            eigvals_n, eigvecs_n = np.linalg.eigh(cov_n)
            dominant_normal = eigvecs_n[:, np.argmax(eigvals_n)]
            
            # Create translation entirely orthogonal to the dominant normal
            t = (np.random.rand(3) - 0.5) * 1.5
            t = t - np.dot(t, dominant_normal) * dominant_normal
            
            T = np.identity(4)
            # Slight rotation around the normal (sliding alignment)
            angle = (np.random.rand() - 0.5) * 0.5
            R = o3d.geometry.get_rotation_matrix_from_axis_angle(dominant_normal * angle)
            T[:3, :3] = R
            T[:3, 3] = t
        else:
